...
public class Tree<L> implements Serializable {
    ...
    public List<Tree<L>> getAtHeight(int height) {
        List<Tree<L>> yield = new ArrayList<Tree<L>>();
        appendAtHeight(height, this, yield);
        return yield;
    }

    private static <L> void appendAtHeight(int height, Tree<L> tree, List<Tree<L>> yield) {
        ...
        for (Tree<L> child : tree.getChildren()) {
            appendAtHeight(height, child, yield);
        }
    }
